...
# Здесь 0 1 * * * означает "Запустить скрипт в 01:00 каждый день" от имени суперпользователя root.
# Замените /path/to/powerOff.sh на фактический путь к вашему скрипту powerOff.sh.
