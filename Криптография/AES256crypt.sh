...
fileEncrypt() {
  ...
  if [[ ! $? ]]; then echo 'При шифровании что-то пошло не так.'; exit 1; fi
...
fileDecrypt() {
  ...
  if [[ ! $?  ]]; then echo 'При расшифровке что-то пошло не так.'; exit 1; fi
...
tar_rf(){
  ...
  if [[ ! $?  ]]; then echo 'При архивации что-то пошло не так.'; exit 1; fi
}
...
if [[ (-n $1) && ($1 == 'dec') ]]
then
  # Удалим последние 4 символа из названия шифрованного файла $1, т.е. удаляем '.enc'
  ...
  then
    # имя изначальной папки, заходим в неё
    cd "$(dirname "$new_file")" || exit 1
    # распаковка архива
    tar_xf "$new_file" 
